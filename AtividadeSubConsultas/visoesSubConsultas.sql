...
--Exercício 08 de SubConsultas
delete from tb_item_pedido
where numpedido in(select numpedido from tb_pedido where codvendedor=32);

--Exercício 09 de SubConsultas
update tb_produto
set valor_unitario = valor_unitario * 0.80
where codproduto not in(select codproduto from tb_item_pedido where numpedido 
in(select numpedido from tb_pedido where to_char(prazo_entrega,'yyyy') =2021));
